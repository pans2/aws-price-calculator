...
$sql = "SELECT * FROM demo10 where type like '%$instance%' and vcpu like '$vcpu' and os like '%$os%' and location like '%$region%' and tenancy like '%$tenancy%' and pre_installedSW like '%$presw%'"; //查询语句--查询数据库表
$result = mysqli_query($conn, $sql);

echo "<table border='1'><tr><td>type</td><td>vcpu</td><td>memory</td><td>location</td><td>tenancy</td><td>OS</td><td>预装软件</td><td>按需每小时价格</td><td>按需每月价格</td><td>按需每年价格</td><td>一年RI价格</td><td>三年RI价格</td>";
